					    ...
					    size_t align, unsigned int flags)
{
	...
	*buffer = (typeof(*buffer)){
	...
	write_lock(&idev->buffer_lock);
	buffer->id = idr_alloc(&idev->buffers, buffer, 1, 0, GFP_NOWAIT);
	write_unlock(&idev->buffer_lock);
	idr_preload_end();
...
struct ion_buffer *ion_buffer_find_by_id(struct ion_device *idev, int id)
{
	...
	read_lock(&idev->buffer_lock);
	buffer = idr_find(&idev->buffers, id);
	read_unlock(&idev->buffer_lock);
				      ...
				      unsigned long arg))
{
	...
	*idev = (typeof(*idev)){
		...
		.buffer_lock = __RW_LOCK_UNLOCKED(idev->buffer_lock),
		.dev = {
