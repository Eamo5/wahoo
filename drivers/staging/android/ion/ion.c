					    ...
					    struct ion_buffer *buffer)
{
	...
	handle->id = idr_alloc(&client->handle_idr, handle, 1, 0, GFP_KERNEL);
	if (handle->id < 0) {
		kfree(handle);
		return ERR_PTR(-ENOMEM);
	}
...
static void ion_handle_put(struct ion_handle *handle)
{
	...
	if (!--handle->refcount) {
		...
		if (!RB_EMPTY_NODE(&handle->node))
			rb_erase(&handle->node, &client->handle_root);
		kfree(handle);
	}
}
			      ...
			      struct ion_handle *handle)
{
	...
	rb_insert_color(&handle->node, &client->handle_root);
}

static struct ion_handle *ion_find_buffer_handle(struct ion_client *client,
						 struct ion_buffer *buffer)
{
	struct rb_node *node = client->handle_root.rb_node;
	struct ion_handle *entry;
...
static long ion_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_ALLOC:
		...
		break;
	case ION_IOC_FREE:
		handle = idr_find(&client->handle_idr, data.handle.handle);
		if (!handle)
			return -EINVAL;
	...
	case ION_IOC_IMPORT:
		...
		handle = ion_find_buffer_handle(client, buffer);
		if (IS_ERR(handle)) {
			handle = ion_handle_create(client, buffer);
			if (IS_ERR(handle)) {
				ion_buffer_put(buffer);
