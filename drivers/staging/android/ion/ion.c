...
static struct ion_handle *ion_handle_get_check_overflow(
					struct ion_handle *handle)
{
	if (!atomic_add_unless(&handle->refcount, 1, -1))
...
{
	if (!atomic_add_unless(&handle->user_ref_count, 1, -1))
		return ERR_PTR(-EOVERFLOW);
	atomic_inc(&handle->refcount);
	return handle;
}
...
ion_handle_lookup_get_check_overflow(struct ion_client *client,
				     struct ion_buffer *buffer)
{
	...
	while (n) {
		...
		if (buffer < entry->buffer) {
			...
			entry = ion_handle_get_check_overflow(entry);
					       ...
					       int fd)
{
	...
	handle = ion_handle_lookup_get_check_overflow(client, buffer);
	if (!IS_ERR(handle) || handle == ERR_PTR(-EOVERFLOW))
		goto end;
