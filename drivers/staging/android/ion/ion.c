...
static void ion_buffer_free_work(struct work_struct *work)
{
	...
	if (client) {
		...
		spin_lock(&client->ubuf_lock);
		list_del(&buffer->lnode);
		free_client = client->dead && list_empty(&client->ubuf_list);
		spin_unlock(&client->ubuf_lock);
		if (free_client)
			kfree(client);

		spin_lock(&idev->ubuf_lock);
		idr_remove(&idev->ubufs, buffer->id);
		spin_unlock(&idev->ubuf_lock);
...
static struct ion_buffer *ion_buffer_alloc(struct ion_device *idev,
					   struct ion_client *client,
					   struct ion_heap *heap, size_t sz,
					   size_t align, unsigned int flags)
{
	...
	*buffer = (typeof(*buffer)){
	...
	if (heap->ops->allocate(heap, buffer, sz, align, flags))
		goto free_buffer;
	...
	if (client) {
		...
		spin_lock(&idev->ubuf_lock);
		buffer->id = idr_alloc(&idev->ubufs, buffer, 1, 0, GFP_NOWAIT);
		spin_unlock(&idev->ubuf_lock);
		idr_preload_end();
		...
		spin_lock(&client->ubuf_lock);
		list_add(&buffer->lnode, &client->ubuf_list);
		spin_unlock(&client->ubuf_lock);
	}
	...
	for (sg = buffer->sg_table->sgl, nents = buffer->sg_table->nents;
	     ...
	     sg = sg_chain_ptr(sg)) {
		for (i = 0; i < SG_MAX_SINGLE_ALLOC - 1; sg++, i++) {
...
void ion_buffer_put(struct ion_buffer *buffer, int count)
{
	if (!atomic_sub_return(count, &buffer->refcount))
		queue_work(system_unbound_wq, &buffer->free);
}
...
struct ion_buffer *ion_buffer_find_by_id(struct ion_client *client, int id)
{
	...
	rcu_read_lock();
	buffer = idr_find(&idev->ubufs, id);
	rcu_read_unlock();

	return buffer ? buffer : ERR_PTR(-EINVAL);
}

struct ion_buffer *__ion_alloc(struct ion_device *idev,
			       struct ion_client *client, size_t sz,
			       ...
			       unsigned int flags)
{
	...
	plist_for_each_entry(heap, &idev->heaps, node) {
		...
		buffer = ion_buffer_alloc(idev, client, heap, sz, align, flags);
		if (!IS_ERR(buffer)) {
...
struct dma_buf *__ion_share_dma_buf(struct ion_buffer *buffer)
{
	...
	if (!IS_ERR(dmabuf))
		atomic_add(0x10000, &buffer->refcount);
...
static long ion_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case ION_IOC_ALLOC:
		buffer = __ion_alloc(idev, client, data.allocation.len,
	...
	case ION_IOC_FREE:
		...
		if (!atomic_add_unless(&buffer->user_refcount, -1, 0))
			ion_buffer_put(buffer, 1);
		return 0;
	case ION_IOC_SHARE:
	case ION_IOC_MAP:
		buffer = ion_buffer_find_by_id(client, data.handle.handle);
	...
	case ION_IOC_IMPORT:
		...
		atomic_inc(&buffer->user_refcount);
		output = &buffer->id;
	...
	if (copy_to_user((void __user *)arg, output, sizeof(*output))) {
		...
		case ION_IOC_MAP:
			...
			break;
		case ION_IOC_IMPORT:
			atomic_dec(&buffer->user_refcount);
			break;
		}
...
static int ion_open(struct inode *inode, struct file *file)
{
	...
	*client = (typeof(*client)){
		.idev = idev,
		.ubuf_list = LIST_HEAD_INIT(client->ubuf_list),
		.ubuf_lock = __SPIN_LOCK_UNLOCKED(client->ubuf_lock)
	};
...
static bool ion_release_buffer(struct ion_buffer *buffer)
{
	int v, v1;

	for (v = 0;; v = v1) {
		v1 = atomic_cmpxchg(&buffer->refcount, v, v & 0xffff0000);
		if (likely(!v1 || v1 == v))
			break;
	}

	return v1 && !(v1 & 0xffff0000);
}
...
static int ion_release(struct inode *inode, struct file *file)
{
	...
	spin_lock(&client->ubuf_lock);
	if (!(free_client = list_empty(&client->ubuf_list))) {
		list_for_each_entry(buffer, &client->ubuf_list, lnode) {
			if (ion_release_buffer(buffer))
				queue_work(system_unbound_wq, &buffer->free);
		}
		client->dead = true;
	}
				      ...
				      unsigned int cmd, unsigned long arg))
{
	...
	*idev = (typeof(*idev)){
		...
		.ubufs = IDR_INIT(idev->ubufs),
		.ubuf_lock = __SPIN_LOCK_UNLOCKED(idev->ubuf_lock),
		.dev = {
